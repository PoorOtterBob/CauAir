...
from src.utils.module.airdualode.DiffAdv_Fusion import Simple_Gated_Fusion as Gated_Fusion


class ODEFunc(nn.Module):
    def __init__(self, gcn_hidden_dim, input_dim, adj_mx, edge_index, edge_attr,
                 K_neighbour, num_nodes, gso, device, num_layers=2, 
    ...
    def laplacian_operator(self):
        ...
        operator.append(
            ...
            #          bias=True)
            ChebGraphConv(c_in=self.input_dim, 
                          c_out=self.gcn_hidden_dim, 
                          Ks=self.K_neighbour, 
                          gso=self.gso)
        )
        ...
        for _ in range(self.num_layers - 2):
            operator.append(
                ...
                #          K=self.K_neighbour, normalization='sym',
                #          bias=True)
                ChebGraphConv(c_in=self.gcn_hidden_dim, 
                          c_out=self.gcn_hidden_dim, 
                          Ks=self.K_neighbour, 
                          gso=self.gso)
            )

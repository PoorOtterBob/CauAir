...
from src.utils.module.airdualode.chebgcn import ChebGraphConv
from src.utils.module.airdualode.utils import load_graph_data
from torch.nn import functional as F

class GNN_Knowledge_Fusion(nn.Module):
    def __init__(self, num_nodes, phy_dim, unk_dim, output_dim, edge_index, edge_attr, gso, 
                 hid_dim=64, gnn_type='Cheb', num_layers=3):
    ...
    def get_fusion(self):
        ...
        if self.gnn_type == "GCN":
            ...
            fusion.append(
                ...
                #          K=3)
                ChebGraphConv(c_in=self.concat_dim, 
                          c_out=self.hid_dim, 
                          Ks=self.K_neighbour, 
                          gso=self.gso)
            )
            ...
            for _ in range(self.num_layers - 2):
                ...
                #              K=3)
                ChebGraphConv(c_in=self.hid_dim, 
                          c_out=self.hid_dim, 
                          Ks=self.K_neighbour, 
                          gso=self.gso)
                )

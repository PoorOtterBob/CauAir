...
import torch
from src.utils.module.airdualode.Explicit_odefunc import ODEFunc
from src.utils.module.airdualode.Diffeq_solver import DiffeqSolver
from typing import Optional
from src.utils.module.airdualode.tools import Air_Attrs
from src.utils.module.airdualode.Encoder import Encoder_unk_z, Coeff_Estimator_new as Coeff_Estimator, Encoder_phy_z
from src.utils.module.airdualode.Decoder import Conv1d_Decoder as Conv_Decoder
from src.utils.module.airdualode.Dynamics_Funsion import GNN_Knowledge_Fusion
from src.utils.module.airdualode.Unk_Dynamics import Unk_odefunc_ATT as Unk_odefunc
from src.utils.module.airdualode.Embed import AirEmbedding
from src.utils.module.airdualode.soft_losses import *
from src.utils.module.airdualode.utils import ConfigDict, load_config

class Model(nn.Module, Air_Attrs):
    def __init__(self, gso, args: Optional[ConfigDict] = None):
        ...
        if self.is_phy:
            self.phy_odefunc = ODEFunc(self.phy_gnn_hid_dim, self.X_dim, self.adj_mx, self.edge_index, self.edge_attr,
                                       self.cheb_k, self.num_nodes, gso, self.device, num_layers=self.phy_gnn_layers,
    ...
    def forward(self, inputs, y_embed=None, gso=None):
        # (X, A)
        # seq_len, batch_size = inputs.size(0), inputs.size(1)
        seq_len, batch_size = inputs.size(1), inputs.size(0)

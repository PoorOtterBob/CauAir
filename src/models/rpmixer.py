...
class RPMixer(BaseModel):
    ...
    def __init__(self, 
        ...
        feat_dim = self.seq_len*(2*self.input_dim - 2)
    ...
    def forward(self, x, label=None):
        ...
        feat = x[..., 1:].transpose(-1, -2).reshape(x.shape[0], -1, self.node_num) # -> (b, t*(f-1), n)
        feat = torch.cat([feat, 
                          label.transpose(-1, -2).reshape(x.shape[0], -1, self.node_num)],
                          dim=1)
